...
gis = GIS("home")
gis_users = gis.users.search(query='username:P00*', max_users=5000, outside_org=0)
...
for user in gis_users:
    ...
    if user.username.find('_browardschools') >=0: 
        now = time.time()
        ...
        if round(diffCreateDays,2) < 2:
            ...
            m=re.search('P00', user.username) # this check is redundant given the original search query string
            if m:  
